...
public class xel_PsionicCrystalArray extends xel_BaseHullmod {
	...
	public void advanceInCombat(ShipAPI ship, float amount) {
		...
		if (engine.getPlayerShip() == ship) {
			String data = null;
			if (ship.getVariant().hasHullMod(HullModUtil.XEL_CYBERNETICS_CORE)) {
				data = i18n_hullmod.get("xel_pca_with_cybernetics_core");
			} else if (ship.getVariant().hasHullMod(HullModUtil.XEL_ARRAY_BATTERY)) {
				data = i18n_hullmod.get("xel_pca_with_array_battery");
			} else if (ship.getVariant().hasHullMod(HullModUtil.XEL_RESONANCE_COIL)) {
				data = i18n_hullmod.get("xel_pca_with_resonance_coil");
			} else return;
	...
	public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
		...
		text.addPara(i18n_hullmod.get("xel_pca_core_upgrade1"), pad, flag ? good : g, "x" + String.format("%.1f", PHASE_DISSIPATION_MULT));
		text.addPara(i18n_hullmod.get("xel_pca_core_upgrade2"), pad, flag ? good : g, "50%");
