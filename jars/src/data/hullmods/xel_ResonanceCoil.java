...
public class xel_ResonanceCoil extends xel_BaseHullmod {
    ...
    //    在峰值时间的前30%内恢复
    //    改善护盾舰护盾抗性，相位舰装甲抗性
    //    内置后 结构抗性 +15%，再改善护盾舰护盾抗性，相位舰装甲抗性
    //    不兼容于重型装甲，护盾分流，硬化护盾
    ...
    private static final float HULL_DAMAGE_TAKEN_BONUS = 15f;
    ...
    public void advanceInCombat(ShipAPI ship, float amount) {
        if (ship.isAlive()) {
            ...
            if (time >= 0f) {
                float level = time / maxRecoveryTime;
    ...
    private static class damageTakenManager implements DamageTakenModifier {
        ...
        private static float getRealBonus(boolean sMod, String dataKey) {
            if (data.get(dataKey) == null) return 0f;
            return 1f + 0.01f * (sMod ? data.get(dataKey).sModBonus : data.get(dataKey).bonus);
        }
        ...
        public String modifyDamageTaken(Object param, CombatEntityAPI target, DamageAPI damage, Vector2f point, boolean shieldHit) {
            ...
            if (shieldHit) {
                if (damageAdjust(damage, damageType, ID, "kh_decrease", "kh_increase")) return ID;
            } else if (isPhase) {
                if (damageAdjust(damage, damageType, ID, "kh_increase", "kh_decrease")) return ID;
            }
            return null;
        }

        private boolean damageAdjust(DamageAPI damage, DamageType damageType, String ID, String kinetic, String highExplosive) {
            if (damageType == DamageType.KINETIC) {
                damage.getModifier().modifyMult(ID, getRealBonus(SMOD, kinetic));
                return true;
            } else if (damageType == DamageType.HIGH_EXPLOSIVE) {
                damage.getModifier().modifyMult(ID, getRealBonus(SMOD, highExplosive));
                return true;
            } else if (damageType == DamageType.ENERGY) {
                damage.getModifier().modifyMult(ID, getRealBonus(SMOD, "ef_decrease"));
                return true;
            } else if (damageType == DamageType.FRAGMENTATION) {
                damage.getModifier().modifyMult(ID, getRealBonus(SMOD, "ef_increase"));
                return true;
            }
            return false;
        }
