...
public class xel_VoidShiftStats extends BaseShipSystemScript {
    public static final String VS_ENERGY_FLAT_BONUS = "xel_vs_energy";

    private static final float VULNERABLE_FRACTION = 0f;
    private static final float MAX_TIME_MULT = 4f;
    private static final float SHIP_ALPHA_MULT = 0.3f;
    private static final float MAX_SPEED_PERCENT = 10f;
    private static final float MAX_TURN_RATE = 50f;
    ...
    private static final float BATTERY_BUFF1 = 2f / 3f;
    private static final float BATTERY_BUFF2 = 10f;
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        float flatBonus = ship.getMutableStats().getDynamic().getValue(VS_ENERGY_FLAT_BONUS);//默认值为1（因为我也不知道怎么用就这么用了
        maxEnergy = 100f + (flatBonus - 1f);
...
//                false);
        ...
        float energyReduced = ENERGY_REDUCED_PER_SEC;
        float energyThreshold = ACTIVE_ENERGY_THRESHOLD;
        float amount = engine.getElapsedInLastFrame();

        // 若有矩阵充能器，恢复加快
        if (ship.getVariant().hasHullMod(HullModUtil.XEL_ARRAY_BATTERY)) {
            energyIncrease *= BATTERY_BUFF1 + 1f;
            energyThreshold -= BATTERY_BUFF2;
        }
        //若有控制芯核
        if (ship.getVariant().hasHullMod(HullModUtil.XEL_CYBERNETICS_CORE)) {
            energyReduced *= CORE_BUFF * 0.01f;
        }
        ...
        if (state == State.COOLDOWN || state == State.IDLE) {
            ...
            energy = Math.min(maxEnergy, energyIncrease * amount + energy); //energy只要在系统不运作就一直增加
        ...
        if (state == State.IN || state == State.ACTIVE) {
            ...
            energy = Math.max(0f, energy - energyReduced * amount);
            ship.setCustomData(dataKey, energy);
            // 启动的判断
            if (!isStarted) {
                isStarted = true;
...
//				DistortionShader.addDistortion(ripple);//生成特效
            ...
            if (ship.getVariant().hasHullMod(HullModUtil.XEL_RESONANCE_COIL)) {
                ...
                float damageReductionLevel = Math.max(0.1f, 1f - effectLevel);
                stats.getHullDamageTakenMult().modifyMult(id, damageReductionLevel);
                stats.getArmorDamageTakenMult().modifyMult(id, damageReductionLevel);
            } else ship.setPhased(effectLevel > 0.5f);
            levelForAlpha = effectLevel;
        }
        // 设置透明度
