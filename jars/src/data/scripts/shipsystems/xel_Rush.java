...
public class xel_Rush extends xel_BaseShipSystemScript {
    /*
    冲锋
...
//                }
			...
			if (target != null) {
				...
				if (chaseAngle < 90f) {
					...
					if (engine.getPlayerShip() == ship) {
						engine.maintainStatusForPlayerShip(STATUS3, "graphics/icons/hullsys/maneuvering_jets.png",
			...
			if (engine.getPlayerShip() == ship) {
				if (!isTakenEffect) {
					engine.maintainStatusForPlayerShip(STATUS3, spriteName,
							title, i18n_shipSystem.get("xel_rush_idle"), true);
				}
				engine.maintainStatusForPlayerShip(STATUS1, spriteName, title, i18n_shipSystem.get("xel_rush1"), false);
				engine.maintainStatusForPlayerShip(STATUS2, spriteName, title, i18n_shipSystem.format("xel_rush2", (int) MAX_SPEED_BONUS), false);
			}
		}
	}
